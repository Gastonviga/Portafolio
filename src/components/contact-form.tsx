...
const WA_NUMBER = "5491171488045"; // reemplazar con número real
...
export function ContactForm({ open, onClose }: ContactFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const mailto = `mailto:gastonvigabriel7@gmail.com?subject=${subject}&body=${body}`;
        ...
        <div className="p-6">
          {sent ? (
            <div className="flex flex-col items-center gap-4 py-6 text-center">
              ...
              <div>
                <p className="font-semibold text-[var(--fg)] mb-1">¡Mensaje listo!</p>
                <p className="text-sm text-[var(--fg-muted)]">
                  Se abrió tu cliente de correo con el mensaje pre-cargado.
                </p>
              </div>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                <button
                  type="submit"
                  disabled={sending}
                  className="group relative flex-1 inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-semibold text-[var(--bg)] bg-[var(--fg)] rounded-full overflow-hidden transition-all duration-300 hover:shadow-[0_0_30px_rgba(232,168,73,0.25)] disabled:opacity-60"
                >
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center justify-center w-11 h-11 rounded-full border border-[var(--border)] text-[var(--fg-muted)] hover:border-emerald-500/50 hover:text-emerald-400 hover:bg-emerald-400/5 transition-all duration-300"
                  >
                  ...
                  {/* Tooltip */}
                  <div
                    className={`absolute bottom-full right-0 mb-2 w-max max-w-[180px] px-3 py-2 bg-[var(--surface-2)] border border-[var(--border)] rounded-xl text-xs text-[var(--fg-muted)] shadow-lg transition-all duration-200 pointer-events-none ${
                      showWaTooltip ? "opacity-100 translate-y-0" : "opacity-0 translate-y-1"
                    }`}
                  >
                    ...
                    {/* Arrow */}
                    <span className="absolute -bottom-1.5 right-3.5 w-3 h-3 bg-[var(--surface-2)] border-r border-b border-[var(--border)] rotate-45" />
                  </div>
                </div>
              </div>
            </form>
